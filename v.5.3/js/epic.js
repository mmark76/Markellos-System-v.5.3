...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateEpicText() {
    ...
    const prologue = ` Ο Γέροντας άνοιξε το παλιό σκονισμένο βιβλίο και είπε στο μικρό μαθητή: "Σήμερα θα μελετήσουμε τη μάχη ανάμεσα σε δύο μεγάλους στρατηγούς: ${white}, που στη μάχη αυτή, έχει το ρόλο του Στρατηγού του Λευκού Στρατού, εναντίον ${black}, που στη μάχη αυτή, έχει το ρόλο του Στρατηγού του Μαύρου Στρατού. Ήταν ${formattedDate}, όταν οι δύο τους συναντήθηκαν στο πεδίο της αναμέτρησης." `;
    
    let finalMsg = "";
    if (result === "1-0") finalMsg = " \n … και μετά την τελευταία κίνηση του λευκού, ο μαύρος Βασιλιάς κατάλαβε πως η μάχη είχε κριθεί. Έσκυψε το κεφάλι του αργά και παραδόθηκε με αξιοπρέπεια. Το πεδίο σώπασε και η ηχώ της μάχης έμεινε να αιωρείται μέσα στο χώρο. Και ο Γέροντας έκλεισε το παλιό βιβλίο και η παρτίδα έμεινε σαν ανάμνηση μιας μάχης που θα ζει για πάντα στη μνήμη και την ιστορία.";
